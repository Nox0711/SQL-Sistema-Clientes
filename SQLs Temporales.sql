...
SELECT *
FROM(
SELECT ITIQUETE,NTEMA,ASUNTO,ESTADO,ASIGNADOA,FHAPERTURA,EstadoTiquete, REPLACE(MIN(DIAS_DESDE_ULTIMO_MENSAJE),'.',',') AS DIAS_DESDE_ULTIMO_MENSAJE ,TIPO_SOLICITUD_APERTURA,OBSERVACIONES
FROM(
SELECT T1.ITIQUETE
,T2.NTEMA	
,T1.ASUNTO
...
ELSE 'Error Revisar SQL'
END) AS EstadoTiquete
,CAST((LOCALTIMESTAMP - T3.FHMENSAJE) AS DECIMAL (10,2))AS DIAS_DESDE_ULTIMO_MENSAJE
...
ORDER BY FHAPERTURA DESC,ASIGNADOA
...
AND T1.FHSOLUCIONADO>='2025-07-18' --FECHA ÚLTIMO SEGUIMIENTO
...
WHERE guid IN ('1021C1CB-B72E-4C9D-BB8B-AEA8A6C9AAB9')
ORDER BY fregistro desc,IACTUALIZACION DESC ,ICOMPILACION DESC
--IRELEASE = 8 AND IACTUALIZACION IN (25) AND ICOMPILACION IN (18,19) and ITERCERO NOT IN ('810000630','900100200','75070023','DEMO-CONTAPYME','DEMO-AGROWIN','63346842','')
