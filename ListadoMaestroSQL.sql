...
,T3.NTEMA
,T1.ABIERTOPOR
,EXTRACT(YEAR FROM T1.FHAPERTURA)||'-'||EXTRACT(MONTH FROM T1.FHAPERTURA)||'-'||EXTRACT(DAY FROM T1.FHAPERTURA) AS FECHA_APERTURA
,CAST((LOCALTIMESTAMP - T1.FHAPERTURA) AS DECIMAL (10,2)) AS DIAS_DESDE_APERTURA
,T5.NTITULO AS TIPO_SOLICITUD_APERTURA
,T1.ASIGNADOA
FROM SOP_TIQUETES T1
LEFT JOIN ABADTIT T5 ON T5.ITABLA='ITDSOLICITUD' AND T1.ITDSOLICITUDA=T5.ITITULO
